		...
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_filter'])) {
				...
				foreach ($filters as $filter_id) {
					$arr = preg_split('/[\[,\]]/',$filter_id);
					if (count($arr) > 1) {
						$where_arr[$arr[0]][] = $arr[1];
					}
				}
				...
				foreach ($where_arr as $key => $value) {
					if ($key == 'manf') {
						...
						$sql .= " LEFT JOIN " . DB_PREFIX . "product_attribute pa" . $key . " ON (p2c.product_id = pa" . $key . ".product_id AND pa" . $key . ".attribute_id = " . $key . ")";
					}
				}
		...
		if (!empty($data['filter_category_id'])) {
			...
			if (!empty($data['filter_filter'])) {
				...
				foreach ($where_arr as $key => $value) {
					if ($key == 'manf') {
						$sql .= " AND (0=1";
						foreach ($value as $wvalue) {
							$sql .= " OR p.manufacturer_id = " . (int)$wvalue;
						}
						$sql .= ")";
					}
					elseif ($key == 'minprice') {
						$sql .= " AND p.price >= " . (float)$value[0];
					}
					elseif ($key == 'maxprice') {
						$sql .= " AND p.price <= " . (float)$value[0];
					}
